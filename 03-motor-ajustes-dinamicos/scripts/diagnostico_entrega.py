...
def ejecutar_diagnostico_critico() -> None:
    ...
    top_5_series: pd.Series = df_critico.groupby("Arquetipo").size().sort_values(ascending=False).head(5)
    
    for arquetipo, total_casos in top_5_series.items():
        pct: float = (total_casos / total_criticos) * 100
        print(f"   -> {arquetipo:<32} | Casos: {total_casos:>3} ({pct:>5.1f}%)")
    print("-" * 50)

    print("2. DESVÍO MONETARIO PROMEDIO POR ARQUETIPO (PEN):")
    desvios_promedio: pd.Series = df_critico.groupby("Arquetipo")["Diferencia"].mean().loc[top_5_series.index]
    ...
    desc_series: pd.Series = df_critico["Descripcion"].astype(str).str.lower()
    
    for kw in KEYWORDS_TECNICAS:
        coincidencias: int = int(desc_series.str.contains(kw, regex=False).sum())
        if coincidencias > 0:
            pct_kw: float = (coincidencias / total_criticos) * 100
